  ...
  return (
    <div
      ...
      >
        ...
        >
          {/* File Header */}
          <div className="bg-[var(--bg)] border-b border-[var(--border)] px-2 sm:px-3 py-1.5 sm:py-2 flex items-center gap-2">
            <div className="flex items-center gap-1 sm:gap-1.5 flex-1 min-w-0">
              <FileCode className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[var(--text-muted)] flex-shrink-0" />
              <span className="text-[10px] sm:text-xs text-[var(--text-muted)] font-mono truncate">
                {useCase.title.toLowerCase().replace(/\s+/g, '-')}.md
              </span>
            </div>
            <div className="flex items-center gap-0.5 sm:gap-1">
              <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-yellow-500/60"></div>
              <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-green-500/60"></div>
              <button className="w-3 h-3 sm:w-3.5 sm:h-3.5 flex items-center justify-center rounded hover:bg-[var(--border)] transition-colors opacity-0 group-hover:opacity-100">
                <X className="w-2 h-2 sm:w-2.5 sm:h-2.5 text-[var(--text-muted)]" />
              </button>
            </div>
          </div>
          
          {/* Card Content */}
          <div className="p-4 sm:p-5 md:p-6 hover:bg-[var(--bg)] transition">
            <div className="text-[var(--primary)] mb-3 sm:mb-4 group-hover:scale-110 transition-transform">
              <div className="scale-90 sm:scale-100">{useCase.icon}</div>
            </div>
            <h3 className="text-lg sm:text-xl font-bold mb-2">{useCase.title}</h3>
            <p className="text-[var(--text-muted)] mb-3 sm:mb-4 text-xs sm:text-sm leading-relaxed">
              {useCase.description}
            </p>
            <ul className="space-y-1.5 sm:space-y-2">
              {useCase.features.map((feature, idx) => (
                <li key={idx} className="text-xs sm:text-sm text-[var(--text-muted)] flex items-center gap-2">
                  <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-[var(--primary)] flex-shrink-0"></span>
                  <span>{feature}</span>
                </li>
              ))}
            </ul>
  ...
  return (
    <section className="relative mt-8">
      ...
      <div className="text-center mb-6 sm:mb-8 px-4 sm:px-6">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight mb-2 sm:mb-3">Built For Every Developer</h2>
        <p className="text-sm sm:text-base md:text-lg text-[var(--text-muted)] max-w-2xl mx-auto">
      ...
      <main
        ref={container}
        className="relative flex w-full flex-col items-center justify-center pb-[100vh] pt-16 sm:pt-24 md:pt-32"
      >
