...
export default function LandingTestimonials() {
  return (
    <motion.section
      ...
      className="mx-auto mt-24 max-w-6xl w-full"
    >
      <div className="mb-8 text-center">
        <h2 className="text-2xl md:text-3xl font-semibold tracking-tight mb-2">Real reflections. Real builders.</h2>
        <p className="text-lg text-[var(--text-muted)] max-w-xl mx-auto">What devs say after logging, rating, and sharing their stacks. The word is out!</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mb-6">
        {TESTIMONIALS.slice(0, 3).map((t, i) => (
          <motion.div
            ...
            className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-7 shadow transition"
          >
            <div className="flex items-center gap-3 mb-2">
              <div className="h-10 w-10 rounded-full border border-[var(--border)] flex items-center justify-center" style={{ background: t.color + '11' }}>
                <Icon icon={t.avatar} width={26} height={26} style={{ color: t.color }} />
              </div>
              <span className="font-semibold text-[var(--text)]">{t.user}</span>
            </div>
            <blockquote className="text-[var(--text-muted)] italic">“{t.quote}”</blockquote>
          </motion.div>
        ))}
      </div>
